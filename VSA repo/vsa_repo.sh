...
create_upstream_repo_file() {
  ...
  local key_list
  # Match all files except DEB-* (Debian keys not needed for RPM systems)
  # This captures: RPM-*, veeam.gpg, 8F4A2F3D, etc, but it will not import invalid keys - that's expected
  key_list=$(printf '%s\n' "${key_index}" | grep -oE '(RPM-[A-Za-z0-9]+|veeam\.gpg|[A-F0-9]{8,16})[^/]*' | grep -v '^DEB-' | sort -u || true)
  [[ -n "${key_list}" ]] || \
...
sync_veeam_keys() {
  ...
  local keys
  # Fetch all key files except DEB-* (Debian keys not needed for RPM systems)
  keys=$(printf '%s\n' "${index}" | grep -oE '(RPM-[A-Za-z0-9]+|veeam\.gpg|[A-F0-9]{8,16})[^/]*' | grep -v '^DEB-' | sort -u || true)
...
setup_lets_encrypt() {
  ...
  [[ "${LE_EMAIL}" != "postmaster@test.us" ]] || fatal "LE_EMAIL is still set to default 'user@test.email'. Please update the CONFIG section with a valid email address for Let's Encrypt certificate expiry notifications."
